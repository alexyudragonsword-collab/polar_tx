...
`tests/android_page_harness.js`（jsdom + 桩 bridge）在 CI 里**执行页面**：
跑 boot、逐个点击运行按钮、断言调用真的到达 bridge。它是补上来的——第一个
上真机的构建**每个运行按钮都没反应**，而纯文本的 parity 检查全部通过，因为
它们只能确认"id 在 HTML 里存在"，看不见控件在运行零点几秒后被
`applyLang()` 的 `textContent` 删掉。带 `data-zh` 的元素不得包含元素，这条
现在由静态不变量 + 这个 harness 两层卡住。
