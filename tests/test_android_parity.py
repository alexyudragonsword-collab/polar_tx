...
CSS = WWW / "style.css"
MANIFEST = WWW.parents[1] / "AndroidManifest.xml"


def read(path: Path) -> str:
    """Read a page asset as UTF-8, explicitly.

    Never ``Path.read_text()`` bare: it uses the LOCALE encoding, which is
    cp1252 on a Windows runner, and every file here carries the app's
    bilingual Chinese strings.  This whole module went red on
    windows-latest and nowhere else for exactly that reason — a decode
    error that says nothing about the thing being tested.
    """
...
def test_the_page_accounts_for_notches_and_gesture_bars():
    ...
    assert "viewport-fit=cover" in read(INDEX)
    assert "env(safe-area-inset-top)" in read(CSS)
